...
| Role | Color |
|---|---|
| Observation, natural flow, or data movement | Muted river blue |
| Deterministic transformation or model | Muted blue-green |
| Agent, policy, or LLM-driven mechanism | Warm sumi gray |
| Action or intervention | Soft ochre or persimmon |
| Validation, risk, or constraint | Deep vermilion, used sparingly |
| Existing neutral path | Warm gray |

Never use color as the only carrier of meaning.
Apply these as category hues rather than background fills. Prefer a calm,
Japanese-inspired tonal family with enough luminance contrast for readable
text; avoid bright saturated purple, electric blue, and neon green.
...
- Define an unfamiliar abbreviation beside its first visible use, or move
  technical parameter inventories to a nearby table.
...
If the figure is editable and exact terminology matters, assign text-free
illustrations to image generation and exact labels/connectors to native objects.
Do not cover generated lettering with an opaque panel and redraw on top.
Compare the candidate with the unchanged accepted figure at matched actual
delivery sizes; test changing a label and moving an illustration and arrow,
then save, reopen and inspect. A passing package test alone is not visual QA.
